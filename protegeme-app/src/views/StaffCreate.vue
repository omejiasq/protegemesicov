...
<form @submit.prevent="onSubmit">
  ...
  <div class="grid">
    ...
    <div class="field">
      <label>Usuario *</label>
      <input v-model="form.usuario" :class="{ error: errors.usuario }" />
      <small v-if="errors.usuario" class="error-text">{{ errors.usuario }}</small>
    </div>
...
function validateForm() {
  ...
  if (!form.value.usuario.trim())          // ✅ nuevo
    errors.value.usuario = 'Ingrese el nombre de usuario'
...
async function onSubmit() {
  ...
  try {
    await staffStore.create({
      usuario: form.value.usuario,        // ✅ ahora usa el campo usuario del form
      ...
      documentNumber: form.value.documentNumber, // ✅ solo el número de documento
    } as any)
    alert('Usuario creado correctamente')
    router.push({ name: 'staff' })
  } catch (e: any) {
    alert(e?.response?.data?.message || 'Error al crear el usuario')
  } finally {
    loading.value = false
  }
}
